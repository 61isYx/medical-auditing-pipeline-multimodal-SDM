...
from src.classifer import *
from src.bootstrap.bootstrap_utils import *
...
if __name__ == "__main__":
    ...
    test_df = test_df[test_df[label_1].isin([1, 0])].reset_index(drop=True)

    train_df = pd.read_parquet("/vol/bitbucket/yl28218/thesis/mimic_cxr_exp/data/train_df_cardio_position.parquet")
    train_df = train_df[train_df[label_1].isin([1, 0])].reset_index(drop=True)
    ...
    for i in tqdm(range(bootstrap_times)):
        seed += 1
        np.random.seed(seed)
        print(f"\nBootstrap iteration {i+1}/{bootstrap_times}, seed={seed}, rare_strength={rare_strength}")
        ...
        test = pd.concat([
            ...
            test_rest.sample(n=270, random_state=seed).reset_index(drop=True)
        ], ignore_index=True).sample(frac=1, random_state=seed).reset_index(drop=True)

        test_small = pd.concat([
            test_underperforming.sample(n=30, random_state=seed).reset_index(drop=True),
            test_rest.sample(n=270, random_state=seed).reset_index(drop=True)
        ], ignore_index=True).sample(frac=1, random_state=seed).reset_index(drop=True)

        test_tiny = pd.concat([
            test_underperforming.sample(n=15, random_state=seed).reset_index(drop=True),
            test_rest.sample(n=285, random_state=seed).reset_index(drop=True)
        ], ignore_index=True).sample(frac=1, random_state=seed).reset_index(drop=True)
        ...
        for key in token_results.keys():
            if len(token_results[key]) > 0:
                ...
                df_tokens.to_parquet(f"bootstrap_results/all_tokens_{key}_exp3.parquet", index=False)
                df_summary = (
                ...
                df_summary.to_csv(f"bootstrap_results/summary_{key}_exp3_2.csv", index=False)

        if len(precision_log) > 0:
            pd.DataFrame(precision_log).to_csv("bootstrap_results/precision_log_exp3_2.csv", index=False)
