...
if __name__ == "__main__":
    ...
    print("=================== Bootstrap Experiment (exp2) ==========")
    ...
    for i in tqdm(range(bootstrap_times)):
        ...
        train = add_noisy_labels(train, label_1, label_2, 0.3, 1, 1, seed)

        # ===== Build test sets (20% / 10% / 5%) =====
        test_underperforming = test_df[(test_df[label_2] == 1) & (test_df[label_1] == 1)].copy()
        test_rest = test_df[(test_df[label_2] != 1) | (test_df[label_1] != 1)].copy()
        ...
        for key in token_results.keys():
            if len(token_results[key]) > 0:
                ...
                df_tokens.to_parquet(f"bootstrap_results/all_tokens_{key}_exp2_2.parquet", index=False)
                df_summary = (
                ...
                df_summary.to_csv(f"bootstrap_results/summary_{key}_exp2_2.csv", index=False)

        if len(precision_log) > 0:
            pd.DataFrame(precision_log).to_csv("bootstrap_results/precision_log_exp2_2.csv", index=False)
